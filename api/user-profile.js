...
export default async function handler(req, res) {
  ...
  // ── Leaderboard — no auth required, token optional (for isYours flag) ───────
  if (req.query?.leaderboard === "true") {
    try {
      const lbType   = req.query?.type   || "tippers";   // tippers | streamers
      const lbPeriod = req.query?.period || "alltime";   // today | weekly | alltime

      // Optionally identify the current user for the "isYours" flag
      const lbToken = req.headers["x-auth-token"] || req.query?.token;
      const lbEmail = lbToken ? (await getEmailFromToken(lbToken)) : null;
      ...
      const entries = (await Promise.all(
        userKeys.map(async (uKey) => {
          try {
            const { result: accRaw } = await kvCommand("GET", uKey);
            const account = parse(accRaw);
            if (!account) return null;

            const acEmail = account.email || uKey.replace("user:", "");
            const { result: actRaw } = await kvCommand("GET", `activity:${acEmail}`);
            const activity = parse(actRaw) || {};

            if (lbType === "tippers") {
              if (account.role !== "viewer") return null;
              let tokens = 0;
              if (lbPeriod === "alltime") {
                tokens = activity.totalSpent || 0;
              } else {
                // Sum from tip history for the period
                const tips = activity.tipHistory || [];
                tokens = tips
                  .filter(t => {
                    if (!t.date) return false;
                    if (lbPeriod === "today")  return dateKey(t.date) === todayStr;
                    if (lbPeriod === "weekly") return isoWeekKey(t.date) === weekStr;
                    return true;
                  })
              ...
              return {
                email:  acEmail,
                name:   account.displayName || account.name || "Viewer",
                avatar: account.avatarImg   || "🦇",
                tokens,
              };
            }

            if (lbType === "streamers") {
              if (account.role !== "streamer") return null;
              const daily = activity.dailyEarnings || [];
              let tokens = 0;
              if (lbPeriod === "alltime") {
                tokens = daily.reduce((s, d) => s + (d.tokens || 0), 0);
              } else if (lbPeriod === "today") {
                tokens = daily.filter(d => d.day === todayStr).reduce((s, d) => s + (d.tokens || 0), 0);
              } else if (lbPeriod === "weekly") {
                tokens = daily.filter(d => isoWeekKey(d.day) === weekStr).reduce((s, d) => s + (d.tokens || 0), 0);
              }
              if (tokens === 0) return null;
              return {
                email:   acEmail,
                ...
                avatar:  account.avatarImg   || "🎭",
                tokens,
                viewers: activity.peakViewers || 0,
              };
            }
      ...
      // Sort descending by tokens, assign rank
      entries.sort((a, b) => b.tokens - a.tokens);
      const ranked = entries.slice(0, 10).map((e, i) => ({
        rank:    i + 1,
        name:    e.name,
        avatar:  e.avatar,
        tokens:  e.tokens,
        viewers: e.viewers || 0,
        isYours: lbEmail ? e.email === lbEmail : false,
      }));

      return res.status(200).json({ ok: true, entries: ranked });
    } catch (err) {
